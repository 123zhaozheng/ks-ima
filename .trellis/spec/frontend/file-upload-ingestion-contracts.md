...
Apply this contract to knowledge file upload, immutable replacement, download, preview, ingestion status, retry/cancel, workspace switching, generated OpenAPI clients, Vue Query state, and desktop/mobile browser tests.
...
- Workspace/document changes abort active XHR and cancel scoped queries. Old workspace progress/history/drafts never appear in the new workspace.
...
| Access revoked/trashed during upload/status | Abort/clear and unavailable state; no stale byte action |
| Workspace switch | Active transfer aborts; old queries/progress/history cleared |
...
3. Desktop Chromium and mobile Chromium Playwright exercise initial upload, progress, completion, ingestion state, retry/cancel, preview/download, failed/incomplete replacement preservation, successful replacement, history advance, workspace switch, and no overlap/overflow.
