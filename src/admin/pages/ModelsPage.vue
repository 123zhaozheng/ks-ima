    ...
    <q-page class="q-pa-lg">
      <div class="row items-center q-col-gutter-md q-mb-md">
        <div class="col">
          ...
          <div class="text-caption text-grey-7">
            {{ t('Central infrastructure and approved workflow versions') }}
          </div>
        </div>
        <q-btn
          flat
      ...
      >
        ...
        >
          <div class="row justify-end q-mb-md">
            <q-btn
              ...
              :label="t('Add gateway')"
              no-caps
              @click="showGatewayForm = true"
            />
          </div>
          <q-card
            v-if="showGatewayForm && canManage"
            flat
            bordered
            class="q-pa-md q-mb-md"
          >
            <div class="text-subtitle1 q-mb-sm">
              {{ gatewayEditId ? t('Edit intranet gateway') : t('New intranet gateway') }}
            </div>
            <div class="row q-col-gutter-md">
              ...
              <q-input
                ...
                dense
                type="password"
                :label="t('Credential (shown once)')"
              />
              <q-toggle
                v-model="gatewayForm.insecurePrivate"
                class="col-12"
                :label="t('Allow explicitly allowlisted private HTTP')"
              />
            </div>
            <div class="q-mt-md">
              <q-btn
                color="primary"
                :loading="saving"
                :label="gatewayEditId ? t('Save gateway') : t('Create disabled gateway')"
                no-caps
                @click="saveGateway"
              />
              <q-btn
          ...
          <q-table
            ...
            row-key="id"
            :loading="loading"
            :no-data-label="t('No gateways configured')"
          >
            <template #body-cell-enabled="props">
              <q-td :props="props">
                <q-toggle
                  ...
                  @update:model-value="toggleGateway(props.row, $event)"
                />
              </q-td>
            </template>
            <template #body-cell-secretPresent="props">
              <q-td :props="props">
                <q-badge :color="props.row.secretPresent ? 'positive' : 'warning'">
                  {{ props.row.secretPresent ? t('Configured') : t('Missing') }}
                </q-badge>
              </q-td>
            </template>
              ...
              >
                <template v-if="canManage">
                  ...
                  <q-btn
                    ...
                    :aria-label="t('Health')"
                    @click="health(props.row.id)"
                  />
                  <q-btn
                    flat
                    dense
                    icon="sym_o_key"
                    :aria-label="t('Rotate credential')"
                    @click="startRotate(props.row.id)"
                  />
                  <q-btn
                    flat
                    dense
                  ...
                  <q-btn
                    ...
                    icon="sym_o_delete"
                    :aria-label="t('Delete')"
                    @click="deleteGateway(props.row.id)"
                  />
                </template>
              </q-td>
            </template>
          ...
          >
            <q-input
              ...
              dense
              type="password"
              :label="t('New credential (shown once)')"
            />
            <q-btn
              class="q-mt-sm"
              color="primary"
        ...
        >
          ...
          >
            <div class="text-subtitle1">
              {{ editingModelId ? t('Edit governed model') : t('Register governed model') }}
            </div>
            <div class="row q-col-gutter-md q-mt-sm">
              <q-select
                v-model="modelForm.gatewayId"
                class="col-12 col-md-3"
                outlined
                dense
                emit-value
              ...
              <q-input
                ...
                outlined
                dense
                :label="t('Remote model name')"
              />
              <q-input
                v-model="modelForm.businessLabel"
                class="col-12 col-md-3"
                outlined
                dense
              ...
              <q-select
                ...
                outlined
                dense
                :options="['chat', 'embedding', 'rerank']"
                :label="t('Capability')"
              />
              <q-input
                v-model.number="modelForm.embeddingDimension"
                class="col-12 col-md-3"
                outlined
                dense
                type="number"
                :label="t('Embedding dimension (if applicable)')"
              />
            </div>
            <q-btn
              class="q-mt-sm"
              color="primary"
              :loading="saving"
              :label="editingModelId ? t('Save model') : t('Create disabled model')"
              no-caps
              @click="saveModel"
            />
            <q-btn
          ...
          <q-table
            ...
            row-key="id"
            :loading="loading"
            :no-data-label="t('No governed models configured')"
          >
            <template #body-cell-validated="props">
              <q-td :props="props">
                <q-badge :color="props.row.validated ? 'positive' : 'warning'">
                  {{ props.row.validated ? t('Validated') : t('Draft') }}
                </q-badge>
              </q-td>
            </template>
            <template #body-cell-enabled="props">
            ...
            <template #body-cell-actions="props">
              ...
              >
                ...
                /><q-btn
                  ...
                  @click="validateModel(props.row.id)"
                /><q-btn
                  flat
                  dense
                  icon="sym_o_delete"
        ...
        >
          ...
          >
            <div class="text-subtitle1">
              {{ t('Create workflow profile') }}
            </div>
            <div class="row q-col-gutter-md q-mt-sm">
              <q-select
                v-model="profileForm.workflow"
                class="col-12 col-md-3"
                outlined
                dense
                :options="workflows"
              ...
              />
              <q-input
                v-model="profileForm.businessAlias"
                class="col-12 col-md-3"
                outlined
                dense
                :label="t('Business alias')"
              />
              <q-input
              ...
              <q-input
                ...
                dense
                type="textarea"
                :label="t('Typed configuration JSON')"
              />
            </div>
            <q-btn
              class="q-mt-sm"
          ...
          <div class="text-caption text-grey-7 q-mb-md">
            {{ t('Published workflow versions are immutable. Clone or edit a draft to change behavior.') }}
          </div>
          <q-list
            bordered
            separator
            ...
            >
              <q-item-section>
                ...
                <q-item-label caption>
                  {{ profile.workflow }} · {{ profile.description }}
                </q-item-label>
              </q-item-section>
              <q-item-section
                v-if="canManage"
              ...
              >
                <div class="row items-center q-gutter-sm">
                  <q-badge>{{ profile.lifecycle }}</q-badge><span v-if="profile.currentVersion">v{{ profile.currentVersion }}</span><q-btn
                    flat
                    dense
                    icon="sym_o_history"
                    :aria-label="t('Versions')"
                    @click="showVersions(profile.id)"
                  /><q-btn
                    flat
                    dense
                    icon="sym_o_verified"
          ...
          <q-card
            v-if="selectedProfile && canManage"
            flat
            bordered
            class="q-pa-md q-mt-md"
          >
            <div class="text-subtitle1">
              {{ t('Draft editor') }} · {{ selectedProfile.businessAlias }}
            </div>
            <q-input
              v-model="draftConfig"
              outlined
              ...
              type="textarea"
              class="q-mt-sm"
              :label="t('Draft configuration JSON')"
            />
            <div class="q-mt-sm q-gutter-sm">
              ...
              /><q-btn
                ...
                :label="t('Validate')"
                no-caps
                @click="validateProfile(selectedProfile.id)"
              /><q-btn
                flat
                :label="t('Publish')"
                no-caps
                @click="publishProfile(selectedProfile.id)"
              /><q-btn
                flat
                :label="t('Disable/restore')"
                no-caps
                @click="toggleProfile(selectedProfile)"
              />
            </div>
          </q-card>
        </q-tab-panel>
        ...
        >
          ...
          >
            ...
            /><q-select
              ...
              emit-value
              map-options
              :options="models.map(model => ({ label: model.businessLabel, value: model.id }))"
              :label="t('Embedding impact model')"
            /><q-btn
              class="col-12 col-md-2"
              color="primary"
              :label="t('Check impact')"
              no-caps
              @click="checkImpact"
            />
          </div>
          ...
          >
            <q-select
              ...
              :label="t('Workflow')"
            /><q-select
              v-model="assignmentProfileId"
              class="col-12 col-md-4"
              outlined
              dense
              emit-value
              map-options
              :options="profiles.filter(profile => profile.workflow === assignmentWorkflow && profile.lifecycle === 'active' && profile.currentVersion > 0).map(profile => ({ label: profile.businessAlias, value: profile.id }))"
              :label="t('Published profile')"
            /><q-input
              v-model.number="assignmentProfileVersion"
              class="col-12 col-md-2"
              outlined
              dense
              type="number"
          ...
          <q-table
            ...
            row-key="workflow"
            :loading="loading"
            :no-data-label="t('No assignments')"
          />
          <q-banner
            v-if="impact"
            class="q-mt-md"
            :class="impact.requiresReindex ? 'bg-orange-1' : 'bg-green-1'"
          >
