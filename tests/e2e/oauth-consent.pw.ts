...
test.describe('real OAuth consent desktop/mobile journey', () => {
  test('denies then approves a registered PKCE request and exchanges the code once', async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('heading', { name: '连接智能体' })).toBeVisible()
    ...
    await expect(page.getByText('此连接可以修改知识内容。')).toHaveCount(0)
    ...
    await expect(page.getByText('此连接可以修改知识内容。')).toBeVisible()
  ...
  test('shows an expired-session affordance without loading grant actions', async ({ page }, testInfo) => {
    ...
    await expect(page.getByText('会话已过期，请登录以继续。')).toBeVisible()
    await expect(page.getByRole('link', { name: '登录' })).toHaveAttribute('href', /\/auth\/sign-in/)
